		...
		>
			<v-col>
				<div class="d-flex mt-9" 
					style="justify-content: center">
					<a href="http://localhost:8000/api/kakao">
						<v-img :src="kakaoLogin" class="login-img1"></v-img>
					</a>
					<a href="http://localhost:8000/api/naver">
...
export default {
	...
	methods: {
		...
		getUserInfo() {
			axios({
				method: 'GET',
				url: `http://localhost:8000/api/${this.$store.state.source}/auth`,
				headers: {
		...
		parseMessage(content) {
			...
			axios({
				method: 'POST',
				url: 'http://localhost:8001/api/tmdb/movies',
				data: payload
			})
				.then((res) => {
					let temp = []
